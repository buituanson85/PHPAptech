...
<?php
    class Product{
        ...
        public function insert_product($data, $files){
            ...
            if ($productName == "" || $category == "" || $brand == ""|| $product_desc == "" || $price == ""|| $sale == "" || $type == ""|| $file_name == "" ){ // kiểm tra người dùng có nhập hay ko
                ...
                $query = "INSERT INTO tbl_product( productName, catID, brandId, product_desc, type, price, sale, image) 
                values('$productName', '$category', '$brand', '$product_desc', '$type', '$price', '$sale', '$unique_image')";
                $result = $this->db->insert ($query);
                if ($result){
                    $alert = "<span class='success'>Add product successfully.</span>";
                    return $alert;
                }else{
                    $alert = "<span style='color: red'>Product add failed.</span>";
                    return $alert;
                }
            }
        }
        ...
        public function update_product($data, $files, $id){
            ...
            if ($productName == "" || $category == "" || $brand == ""|| $product_desc == "" || $price == "" || $sale == "" || $type == ""){ // kiểm tra người dùng có nhập hay ko
                $alert = "<span class='error' style='color: red'>Fiels must be not empty</span>";
                return $alert;
            }else{
                if (!empty($file_name) && !empty($file_name_2)){//nếu người dùng chọn ảnh
                    if ($file_size > 1000000){
                        $alert = "<span style='color: red'>Image size no larger than 2MB!</span>";
                        return $alert;
                    }
                    elseif (in_array ($file_ext, $permited)){
//                    echo "<span class='error'>You can uploads only:-".implode (', ', $permited)."</span>";
                        $alert = "<span style='color: red'>You have to uploat the photo with the tail: ".implode (', ', $permited)."</span>";
                        return $alert;
                    }
                    move_uploaded_file ($file_temp, $uploaded_image);
                ...
                where productId = '$id'";
                    ...
                    if ($result){
                        $alert = "<span class='success' style='color: green'>Update successful.</span>";
                        return $alert;
                    }else{
                        $alert = "<span style='color: red'>Update failed</span>";
                        return $alert;
                    }
        ...
        public function del_product ($id){
            ...
            if ($result){
                ...
                $alert = "<span style='color: red'>Product deletion failed.</span>";
                return $alert;
            }
        }
