    ...
    <div class="col-sm-4 mb-5">
        ...
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                ...
                echo 'ID yêu cầu : '. $row['request_id'];
                echo '</div>';
                echo '<div class="card-body">';
                echo '<h5 class="card-title">Thông tin yêu cầu : ' . $row['request_info'] . '</h5>';
                echo '<p class="card-text">' . $row['request_desc'] . '</p>';
                echo '<p class="card-text">Ngày gửi: ' . $row['request_date'] . '</p>';
      ...
      <h4 class="alert-heading">Làm tốt!</h4>
      <p>Rất tiếc, bạn đã chỉ định thành công tất cả các Yêu cầu.</p>
      <hr>
      <h5 class="mb-0">Không có yêu cầu đang chờ xử lý</h5>
    </div>';
        }
        ...
        if(isset($_REQUEST['close'])){
            ...
            if($conn->query($sql) === TRUE){
                ...
                echo "Không thể xoá dữ liệu này";
            }
        }
        ?>
